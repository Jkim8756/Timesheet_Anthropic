    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            try:
                ...
                # Response was truncated — salvage whatever valid JSON we can
                log.warning(
                    f"[{image_path.name}] Response truncated, attempting partial recovery..."
                )
                try:
                    salvaged = raw
                    ...
                    result["_warning"] = (
                        "Response truncated - some records may be missing"
                    )
                    log.warning(f"[{image_path.name}] Partial recovery succeeded.")
                    return result
                except json.JSONDecodeError as e:
                    log.warning(
                        f"[{image_path.name}] Partial recovery failed attempt {attempt}: {e}"
                    )

        except anthropic.RateLimitError:
            log.warning(
                f"[{image_path.name}] Rate limited - waiting {RETRY_DELAY * attempt}s"
            )
...
def build_excel(all_results: list[dict], output_path: Path):
    ...
    for result in all_results:
        ...
        for staff_type, s in all_staff:
            ...
            row_fill = (
                flag_fill if confidence < 0.8 else (alt_fill if row % 2 == 0 else None)
            )
            ...
            values = meta + [
                ...
                (
                    "Yes"
                    if s.get("absent")
                    else ("No" if s.get("absent") is False else "")
                ),
                (
                    "Yes"
                    if s.get("schedule_changed")
                    else ("No" if s.get("schedule_changed") is False else "")
                ),
                confidence,
    ...
    ws2["A4"] = "Successful:"
    ws2["B4"] = (
        f"=COUNTIF('All Records'!V:V,\"OK\")+COUNTIF('All Records'!V:V,\"⚠ Low Confidence\")"
    )
    ws2["A5"] = "Failed files:"
    ...
    for result in all_results:
        if result["_status"] == "failed":
            ...
            for staff in (result.get("frontline_staff") or []) + (
                result.get("management_staff") or []
            ):
                if (staff.get("confidence") or 1.0) < 0.8:
                    ...
                    ws3.cell(
                        row=r,
                        column=2,
                        value=f'Low confidence: {staff.get("employee_name","")} ({staff.get("confidence","")})',
                    )
                    ws3.cell(row=r, column=3, value=result["_extracted_at"])
                    r += 1
...
def run_batch(
    ...
    # Check if output Excel is already open before spending any credits
    ...
    for i, img_path in enumerate(images, 1):
        ...
        staff_count = len(result.get("frontline_staff") or []) + len(
            result.get("management_staff") or []
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Batch timesheet extractor using Claude Vision"
    )
    parser.add_argument("input_folder", help="Folder containing timesheet images")
    parser.add_argument(
        "--output", default="timesheet_output.xlsx", help="Output Excel filename"
    )
    parser.add_argument("--json", default="results.json", help="JSON backup filename")
    parser.add_argument(
        "--db", default=None, help="PostgreSQL URL for DB export (optional)"
    )
    parser.add_argument(
        "--delay",
        type=float,
